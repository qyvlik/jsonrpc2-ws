        ...
        ws.on('close', async () => {
            if (!state.done) {
                ...
                for (let [_, callback] of jsonRpc.callbacks) {
                    await callback({error: {code: JSON_RPC_ERROR_LOST_CONNECTION, message: 'Lost connection!'}});
                }
...
export default class JsonRpcWS {
    ...
    static async startupServer(jsonRpc, port) {
        ...
        webSocketServer.on('connection', async (websocket, request) => {
            ...
            if (jsonRpc.onConnectionInterceptor && !await jsonRpc.onConnectionInterceptor.previous(websocket, request)) {
            ...
            websocket.on('message', async (data, isBinary) => {
                ...
                const messageObject = safeParseJSON(data);
                ...
                const isRequest = messageObject.method !== 'undefined';
